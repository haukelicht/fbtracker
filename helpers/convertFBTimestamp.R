...
convertFBTimstamp <- function(fb.timestamp, 
  ...
  if (convert.to.date) {
    ...
    date <- strftime(fb_date, format = date.format)
    return(date)
    ...
    timestamp <- strftime(gsub("-", "/", fb_timestamp), format = timestamp.format)
    return(timestamp)
  }
